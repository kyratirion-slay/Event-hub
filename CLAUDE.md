...
- Dashboard (`app/page.tsx`) heeft een `CommAlertsBlock`: reminders verschijnen pas **op de dag zelf** (of wanneer te laat; rood = te laat, amber = vandaag) en blijven staan tot de gebruiker afvinkt, wegklikt (kruisje → `dismissed`) of uitstelt (+1/+3/+7 dagen → `snoozeUntil`). Datum van een stap wijzigen reset `dismissed`/`snoozeUntil`. Er is geen server, dus geen e-mail/push.
